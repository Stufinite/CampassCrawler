...
col_namekey = {
    ...
    "開課人數":"number",
    "選課人數":"enrolled_num",
    "可加選餘額":"remaining_seat",    
    "語言":"language",
    "備註":"note",        
    "(六月預選)開放人數":"available",
    "(六月預選)目前登記抽籤人數":"registered_num",
    "(六月預選)中籤率":"possibility",
    "(新生預選)`開放人數":"available",
    "(新生預選)`目前登記`抽籤人數":"registered_num",
    "(新生預選)`中籤率":"possibility"
}
obligatorytf = {
    "必修":True,
    "選修":False
...
int_field = ["number","credits"]
def get_nchu_course(url, payload):
    response = requests.post(url, data=payload)
    datas = []
    for table in re.findall(r'<TABLE  border=1 width="1354"  cellspacing="0" cellpadding="0" class="word_13">\n<TR>\n<TD ALIGN="center" width=30>必選別</TD>.*?</TABLE>', response.text, re.S):#re:正規表示法，findallc會回傳所有不重疊的搜尋結果
        #r表示裡面放的是正規表示法，第二個參數放的是字串，re.S表示flag，他讓.的定義也包含了'\n'
        data = {}#這個dict裡面存的是系所名稱+年級+班別
        table = table.replace("</BR>","`").replace("\u3000",'').replace("<br>",'`').replace("</br>",'`').replace("<FONT COLOR='#FF0000'>",'').replace("</FONT>",'').replace(",",'`')
        #取代字串，\u3000為全形空白、</br>、<br>這些有可能會出現在上課時間，
        #之後跟'，'一起變成'`'，在一起被切開        
        d = pq('<div>'+table+'</div>')#PyQuery的建構式就是可以直接接受一個xml的是串或是html檔
        #d與pq都等同於$，不過要先經由pq()先讓它抓到html文件
        table_title = d('TD:contains("必選別")').text()#d就是jQuery的$
        if table_title == '':
            continue
...
if __name__ == "__main__":
    ...
    try:
        ...
        for ID in dept_id:
            raw = get_nchu_course(url,{'v_check':'1','v_subject':ID})
            data = []
